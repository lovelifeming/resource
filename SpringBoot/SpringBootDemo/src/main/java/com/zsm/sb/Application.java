...
import com.zsm.sb.sensitive.SensitiveInfoUtils;
import com.zsm.sb.sensitive.SensitiveToString;
 ...
 * 第一种是 classpath:/application.properties  在jar包的同一目录下建一个config文件夹，然后把配置文件放到这个文件夹下；
 * 第二种是 file:/application.properties    直接把配置文件放到jar包的同级目录；
 * 第三种是 /config/config.properties    在classpath下建一个config文件夹，然后把配置文件放进去；
 * 第四种是     在classpath下直接放配置文件。
 */
@PropertySource(value = {"classpath:/config/application.properties", "file:/application.properties",
